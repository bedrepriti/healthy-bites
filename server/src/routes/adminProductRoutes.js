// server/src/routes/adminProductRoutes.js
  ...
  try {
    ...
    db.query(
      `INSERT INTO products (name, price, category, tags, image, image_public_id, stock_qty, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, Number(price), category, tags || null, imageUrl, publicId, Number(stock_qty || 0), Number(is_active ?? 1)],
      (err, result) => {
        if (err) return res.status(500).json({ message: "DB error", error: err.message });
        res.json({ message: "Product added ✅", id: result.insertId, image: imageUrl, image_public_id: publicId });
      }
    );
  } catch (e) {
...
router.put("/products/:id", requireAdmin, upload.single("image"), async (req, res) => {
  ...
  if (!name || !price || !category) return res.status(400).json({ message: "Missing fields: name, price, category" });
    ...
    db.query(
      `UPDATE products
       SET name=?, price=?, category=?, tags=?,
           ...
           stock_qty=?, is_active=?
       WHERE id=?`,
      [name, Number(price), category, tags || null, newImageUrl, newPublicId, Number(stock_qty || 0), Number(is_active ?? 1), id],
      (err, result) => {
        if (err) return res.status(500).json({ message: "DB error", error: err.message });
        if (!result.affectedRows) return res.status(404).json({ message: "Product not found" });
        res.json({ message: "Product updated ✅", image: newImageUrl || undefined, image_public_id: newPublicId || undefined });
      }
    );
  } catch (e) {
